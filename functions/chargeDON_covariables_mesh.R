...
mesh = inla.mesh.2d(loc=rbind(as.matrix(collision[,c("X","Y")]),as.matrix(vivant[,c("x","y")]))/1000,
                    boundary = list(bndint,bndext),max.edge = c(3,15),cutoff = c(3,10),
...
w <- sapply(1:length(dmesh), function(i) {
  if (gIntersects(dmesh[i, ], domaineSP))
    return(gArea(gIntersection(dmesh[i, ], domaineSP)))
  else return(0)
})
...
dmeshROUTE <- book.mesh.dual(meshROUTE)

wROUTE <- sapply(1:length(dmeshROUTE), function(i) {
  if (gIntersects(dmeshROUTE[i, ], routeBUF))
    return(gArea(gIntersection(dmeshROUTE[i, ], routeBUF)))
  else return(0)
  })
...
resopred=80
pxlVIV <- pixels(mesh, mask=domaineSP, nx = resopred, ny = round(resopred*yrange/xrange))
projgridVIV <- inla.mesh.projector(mesh, coordinates(pxlVIV))
resopred=300
pxlMOR <- pixels(meshROUTE, mask=routeBUF, nx = resopred, ny = round(resopred*yrange/xrange))
projgridMOR <- inla.mesh.projector(meshROUTE, coordinates(pxlMOR))

save.image("OUT_chargeDON_covariables_mesh.rda")
