#-----------------------------------------------------------------------#
#### This script computes the figures presented in the main text
...
# anonymized until review process completed by the editor's request
# the formatting (not the script) has been partly performed with chatGPT
#----------------------------------------------------------------------#
...
source("functions/covariables_mesh.R")
...
save(NobsMOR_res0, AUCtot, TSStot, PREDtot, R2tot, CORtot, file = "metricsFIT.rda")
...
for (gsp in unique(listeSP[, "grpSP"])) {
  ...
  obs_MORT <- st_sf(as.data.frame(cbind(Nobs_MORT = Nobs_MORT)), geometry = st_geometry(pxlMOR$pixels_sf))
  ...
  for (j in 1:100) {
    ...
    totoobs  <- rbind(totoobs, cbind(as.numeric(rownames(obs_remove)), obs_remove$Nobs_MORT))
    ...
    if (length(unique(NobsMOR_bis)) > 1) {
      ...
      AUC <- c(AUC, testroc$auc)
      TSS <- c(TSS,
               max(testroc$sensitivities + testroc$specificities - 1))
    } else {
      AUC <- c(AUC, NA)
      TSS <- c(TSS, NA)
    }
    ...
    COR <- c(COR,
             cor(results[[j]]$pred_remove$predMORT_val, obs_remove$Nobs_MORT))
  }
...
for (gsp in unique(listeSP[, "grpSP"])) {
  ...
  p1PRED <- ggplot(data.frame(AUC = AUC), aes(x = AUC)) +
    geom_histogram(binwidth = 0.0218,
                   fill = "gray80",
                   color = "black") +
  ...
  p3PRED <- ggplot(summary_pred, aes(x = OBS, y = PRED_50)) +
    geom_point(size = 2, color = "black") +
    geom_errorbar(aes(ymin = PRED_2.5, ymax = PRED_97.5),
                  width = 0,
                  color = "black") +
    geom_abline(
      slope = 1,
  ...
  row1_title <- textGrob("Model fit",
                         rot = 90,
                         gp = gpar(fontface = "bold", fontsize = 12))
  row2_title <- textGrob("Cross-validation",
                         rot = 90,
                         gp = gpar(fontface = "bold", fontsize = 12))
  
  png(
    file = paste0("eval_", gsp, ".png"),
    width = 300,
    height = 200,
    res = 300,
    units = "mm"
...
for (gsp in unique(listeSP[, "grpSP"])) {
  ...
  pp <- ggplot(df, aes(x = Nobs, y = pred)) +
    ...
    labs(
      x = "Observed",
      y = "Predicted",
      title = paste0("AUC = ", round(AUCviv, 3), "    TSS = ", round(TSSviv, 3)),
      subtitle = sprintf("Regression: y = %.3f + %.3f × x", unname(coef(mod)[1]), unname(coef(mod)[2]))
    ) +
...
for (gsp in unique(listeSP[, "grpSP"])) {
  ...
  rownames(toto) <- c(
    "Intercept",
    "Agricultural landscapes vs. forests",
    "Rural vs. urban",
...
df_all$Parameter <- factor(
  ...
  labels = c(
    "(a) Agricultural landscapes vs. forests",
    "(b) Rural vs. urban",
    "(c) Density of watercourses",
    "(d) Bocages vs. open fields",
    "(e) Sampling effort"
  )
)
...
pp <- ggplot(df_all, aes(x = mean, y = Species, color = pp_bin)) +
  ...
  geom_point(size = 2) +
  scale_color_manual(values = bin_colors, name = "Posterior Probability\n(effect > 0)") +
...
final_plot <- pp_nolegend + inset_element(
  legend,
  left = 0.7,
  bottom = 0.05,
  right = 0.8,
...
make_ci_polygon_df <- function(d) {
  ...
  n <- nrow(d)
  data.frame(x = c(d$`0.975quant`, rev(d$`0.025quant`)),
...
all_sp <- list(
  lapply(Efaune, function(x) {
    ...
    proba <- mean(x$probpost, na.rm = TRUE)
    c(qEff, proba)
  }),
...
EffectsMORT <- lapply(common_names, function(name) {
  do.call(rbind, lapply(all_sp, function(lst)
    lst[[name]]))
})
...
EffectsMORT <- lapply(EffectsMORT, function(x) {
  rownames(x) <- c("Exposure",
                   "Speed",
                   "Traffic",
                   "Distance to water",
                   "Distance to vegetation")
  colnames(x) <- c("0.025quant",
                   "median",
                   "0.975quant",
...
if (is.null(names(all_sp_polys)) ||
    any(names(all_sp_polys) == "")) {
...
poly_df <- poly_df %>%
  mutate(
    Parameter = factor(Parameter, labels = param_labels),
...
poly_df <- poly_df %>%
  ...
  ungroup() %>%
  mutate(sp_row = sp_index[as.character(Species)], y_plot = sp_row + band * y_std)
...
pp_df <- do.call(rbind, lapply(names(EffectsMORT), function(sp) {
  ...
  data.frame(
    Species = sp,
    Parameter = if (!is.null(rownames(df)))
      rownames(df)
    else
      rownames(df <- transform(df, Parameter = rownames(df))),
...
poly_df$Parameter <- factor(
  ...
  labels = c(
    ...
    "(c) Traffic",
    "(d) Distance to water",
    "(e) Distance to vegetation"
  )
)

# Plot
p <- ggplot(poly_df, aes(
  x = x,
  y = y_plot,
  group = interaction(Species, Parameter)
)) +
  geom_vline(xintercept = 0,
             linetype = "dashed",
             color = "grey70") +
  geom_polygon(aes(fill = pp_bin, color = pp_bin),
               alpha = 0.35,
               linewidth = 0.2) +
  facet_wrap( ~ Parameter, scales = "free_x", ncol = 3) +
  scale_y_continuous(
    breaks = sp_index,
    labels = names(sp_index),
...
final_plot <- p_nolegend + inset_element(
  legend,
  left = 0.7,
  bottom = 0.05,
  right = 0.8,
